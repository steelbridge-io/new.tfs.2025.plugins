...
add_action('save_post', 'sbm_schoolv3_meta_save');
function sbm_schoolv3_meta_save($post_id)
{

  // Checks save status
  $is_autosave = wp_is_post_autosave($post_id);
  $is_revision = wp_is_post_revision($post_id);
  $is_valid_nonce = (isset($_POST['schoolv3_nonce']) && wp_verify_nonce($_POST['schoolv3_nonce'], basename(__FILE__))) ? 'true' : 'false';
  ...
  // Checks for input and sanitizes/saves if needed
  if (isset($_POST['dest-schoolv3-logo'])) {
    update_post_meta($post_id, 'dest-schoolv3-logo', esc_url_raw($_POST['dest-schoolv3-logo']));
  }

  // Save Travel Hero Video URL
  if (isset($_POST['schoolv3-hero-video'])) {
    $hero_video_url = trim((string)$_POST['schoolv3-hero-video']);
    if ($hero_video_url === '') {
      delete_post_meta($post_id, 'schoolv3-hero-video');
    } else {
      update_post_meta($post_id, 'schoolv3-hero-video', esc_url_raw($hero_video_url));
    }
  }

  // Checks for input and sanitizes/saves if needed
  if (isset($_POST['schoolv3-costs-image'])) {
    update_post_meta($post_id, 'schoolv3-costs-image', esc_url_raw($_POST['schoolv3-costs-image']));
  }
  ...
  // Checks for input and sanitizes/saves if needed
  if (isset($_POST['schoolv3-description'])) {
    update_post_meta($post_id, 'schoolv3-description', sanitize_text_field($_POST['schoolv3-description']));
  }
  ...
  // Checks for input and saves if needed
  if (isset($_POST['feature-1-schoolv3ins-textarea'])) {
    update_post_meta($post_id, 'feature-1-schoolv3ins-textarea', wp_kses($_POST['feature-1-schoolv3ins-textarea'], $allowed_html));
  }
  ...
  // Checks for input and sanitizes/saves if needed
  if (isset($_POST['schoolv3-seasons-image'])) {
    update_post_meta($post_id, 'schoolv3-seasons-image', esc_url_raw($_POST['schoolv3-seasons-image']));
  }
  ...
  // Checks for input and saves if needed
  if (isset($_POST['feature-5-itinerary-title'])) {
    update_post_meta($post_id, 'feature-5-itinerary-title', wp_kses($_POST['feature-5-itinerary-title'], $allowed_html));
  }

  // Checks for input and saves if needed
  if (isset($_POST['feature-5-itinerary-img'])) {
    update_post_meta($post_id, 'feature-5-itinerary-img', esc_url_raw($_POST['feature-5-itinerary-img']));
  }
  ...
  // Checks for input and saves if needed
  if (isset($_POST['feature-5-itinerary-content'])) {
    update_post_meta($post_id, 'feature-5-itinerary-content', wp_kses($_POST['feature-5-itinerary-content'], $allowed_html));
  }
  ...
  // Checks for input and saves if needed
  if (isset($_POST['feature-5-itinerary-readmore'])) {
    update_post_meta($post_id, 'feature-5-itinerary-readmore', wp_kses($_POST['feature-5-itinerary-readmore'], $allowed_html));
  }

	// Checks for input and saves if needed - Species Fields
	if (isset($_POST['feature-6-supplemental-title'])) {
	 update_post_meta($post_id, 'feature-6-supplemental-title', wp_kses($_POST['feature-6-supplemental-title'], $allowed_html));
	}

	// Checks for input and saves if needed
	if (isset($_POST['feature-6-supplemental-img'])) {
	 update_post_meta($post_id, 'feature-6-supplemental-img', esc_url_raw($_POST['feature-6-supplemental-img']));
	}
	...
	// Checks for input and saves if needed
	if (isset($_POST['feature-6-supplemental-content'])) {
	 update_post_meta($post_id, 'feature-6-supplemental-content', wp_kses($_POST['feature-6-supplemental-content'], $allowed_html));
	}

	// Checks for input and saves if needed
	if (isset($_POST['feature-6-supplemental-readmore'])) {
	 update_post_meta($post_id, 'feature-6-supplemental-readmore', wp_kses($_POST['feature-6-supplemental-readmore'], $allowed_html));
	}

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image1'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image1',
      $_POST['additional-schoolv3-image1']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image1-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image1-link',
      $_POST['additional-schoolv3-image1-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image2'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image2',
      $_POST['additional-schoolv3-image2']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image2-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image2-link',
      $_POST['additional-schoolv3-image2-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image3'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image3',
      $_POST['additional-schoolv3-image3']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image3-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image3-link',
      $_POST['additional-schoolv3-image3-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image4'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image4',
      $_POST['additional-schoolv3-image4']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image4-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image4-link',
      $_POST['additional-schoolv3-image4-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image5'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image5',
      $_POST['additional-schoolv3-image5']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image5-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image5-link',
      $_POST['additional-schoolv3-image5-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image6'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image6',
      $_POST['additional-schoolv3-image6']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image6-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image6-link',
      $_POST['additional-schoolv3-image6-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image7'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image7',
      $_POST['additional-schoolv3-image7']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image7-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image7-link',
      $_POST['additional-schoolv3-image7-link']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image8'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image8',
      $_POST['additional-schoolv3-image8']);
  }

  // Checks for input and saves if needed
  if (isset($_POST['additional-schoolv3-image8-link'])) {
    update_post_meta($post_id,
      'additional-schoolv3-image8-link',
      $_POST['additional-schoolv3-image8-link']);
  }
