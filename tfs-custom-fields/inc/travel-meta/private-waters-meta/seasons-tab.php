<?php
// Private Waters Seasons Tab (aligned with Destination V3 layout, with PW-specific fixes)
// Expect $post and $sbm_stored_travel_meta to be available from the parent meta callback.
?>
...
<div class="seasons-container">
 ...
 <div class="meta-field-container">
  ...
  <input
   ...
   ?>"
  />
  <p class="description"><?php _e( 'Add a direct video URL (e.g., MP4). Playback is user-initiated via controls. Poster uses the Seasons image if set.', 'tfs-travel-textdomain' ); ?></p>
 </div>

 <?php
 $feature_2_video_current = isset( $sbm_stored_travel_meta['feature_2_video_url'] )
  ? trim( (string) $sbm_stored_travel_meta['feature_2_video_url'][0] )
  : '';
    ...
    <div class="tab-content">
     <div role="tabpanel" class="tab-pane fade in active" id="basicseason">
      <p>
       ...
       <textarea name="feature-2-seasons-content" id="feature-2-seasons-content" style="width: 100%; min-height: 140px; "><?php if ( isset ( $sbm_stored_travel_meta['feature-2-seasons-content'] ) ) echo $sbm_stored_travel_meta['feature-2-seasons-content'][0]; ?></textarea>
      </p>
      <p>
       <strong><label for="feature-2-read-more-info" class="sbm-row-title"><?php _e( 'Read More Content (Hidden overlay)', 'tfs-travel-textdomain' )?></label></strong>
       <textarea name="feature-2-read-more-info" id="feature-2-read-more-info" style="width: 100%; min-height: 140px; "><?php if ( isset ( $sbm_stored_travel_meta['feature-2-read-more-info'] ) ) echo $sbm_stored_travel_meta['feature-2-read-more-info'][0]; ?></textarea>
      </p>
      <p>
       <strong><label for="feature-2-seasons-readmore" class="sbm-row-title"><?php _e( 'Read More Link Text', 'tfs-travel-textdomain' )?></label></strong>
       <input style="width: 100%;" type="text" name="feature-2-seasons-readmore" id="feature-2-seasons-readmore" value="<?php if ( isset ( $sbm_stored_travel_meta['feature-2-seasons-readmore'] ) ) echo $sbm_stored_travel_meta['feature-2-seasons-readmore'][0]; ?>" />
      </p>
     </div>
     ...
     <div role="tabpanel" class="tab-pane fade" id="hilowseason">
      <div class="checkbox">
       <label>
        ...
        <?php _e('Enable High / Low Season', 'tfs-travel-textdomain'); ?>
       </label>
      </div>
      <p>
       <strong><label for="feature-2-seasons-hi-lo-content" class="sbm-row-title"><?php _e( 'High / Low Season Content', 'tfs-travel-textdomain' )?></label></strong>
       <textarea name="feature-2-seasons-hi-lo-content" id="feature-2-seasons-hi-lo-content" style="width: 100%; min-height: 140px; "><?php if ( isset ( $sbm_stored_travel_meta['feature-2-seasons-hi-lo-content'] ) ) echo $sbm_stored_travel_meta['feature-2-seasons-hi-lo-content'][0]; ?></textarea>
      </p>
      <p>
       <strong><label for="feature-2-seasons-hiseason" class="sbm-row-title"><?php _e( 'High Season', 'tfs-travel-textdomain' )?></label></strong>
       <textarea name="feature-2-seasons-hiseason" id="feature-2-seasons-hiseason" style="width: 100%; min-height: 140px; "><?php if ( isset ( $sbm_stored_travel_meta['feature-2-seasons-hiseason'] ) ) echo $sbm_stored_travel_meta['feature-2-seasons-hiseason'][0]; ?></textarea>
      </p>
      <p>
       <strong><label for="feature-2-seasons-lowseason" class="sbm-row-title"><?php _e( 'Low Season', 'tfs-travel-textdomain' )?></label></strong>
       <textarea name="feature-2-seasons-lowseason" id="feature-2-seasons-lowseason" style="width: 100%; min-height: 140px; "><?php if ( isset ( $sbm_stored_travel_meta['feature-2-seasons-lowseason'] ) ) echo $sbm_stored_travel_meta['feature-2-seasons-lowseason'][0]; ?></textarea>
      </p>
     </div>
     ...
     <div role="tabpanel" class="tab-pane fade" id="monthlyrange">
      <div class="checkbox">
       <label>
        ...
        <?php _e('Enable Monthly Range Calendar', 'tfs-travel-textdomain'); ?>
       </label>
      </div>

      <p>
       <strong><label for="multi-season-calendar-title" class="sbm-row-title"><?php _e( 'Calendar Title', 'tfs-travel-textdomain' )?></label></strong>
       <input style="width: 100%;" type="text" name="multi-season-calendar-title" id="multi-season-calendar-title" value="<?php if ( isset ( $sbm_stored_travel_meta['multi-season-calendar-title'] ) ) echo esc_attr( $sbm_stored_travel_meta['multi-season-calendar-title'][0] ); ?>" />
      </p>

      <?php
      // Reusable months array
         ...
         document.addEventListener('DOMContentLoaded', function() {
             ...
             function updateMultiSeasonPreview() {
                 ...
                 for (let month = 1; month <= 12; month++) {
                     ...
                     // Now set backgrounds for each period based on contributors
                     const periods = ['early', 'mid', 'late'];
                     periods.forEach(period => {
             ...
             // Hook up change listeners to inputs
